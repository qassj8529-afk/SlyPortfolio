...
export interface TextDrawLinkProps {
  text: string;
  href: string;
  className?: string;
}
...
export const TextDrawLink: React.FC<TextDrawLinkProps> = ({ text, href, className }) => {
  ...
  const handleMouseEnter = () => {
    ...
    const newSvgData = pathDataVariants[globalNextIndex % pathDataVariants.length];
    globalNextIndex++;
  ...
  return (
    <Link href={href} className={cn("relative inline-block py-1 cursor-pointer", className)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <span className="relative z-10">{text}</span>
      <div className="absolute left-0 right-0 bottom-0 h-[15px] w-full pointer-events-none">
        {currentSvg && (
          <svg
          ...
          >
            <path ref={pathRef} d={currentSvg.d} stroke="#6c5ce7" strokeWidth="4" strokeLinecap="round" />
          </svg>
        )}
      </div>
    </Link>
